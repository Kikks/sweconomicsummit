...
const WhyAttend = () => {
  ...
  return (
    <section className="w-full px-5 pb-40 pt-20 md:px-10 lg:px-20">
      <div ref={ref} className="container">
        ...
        <motion.div
          ...
          className="mt-10 grid w-full grid-cols-1 gap-x-2 gap-y-5 sm:grid-cols-2 md:gap-y-20"
        >
          {whyAttendLinks.map((item, index) => (
            <motion.div key={index} variants={linkVariant}>
              ...
              >
                <span className="self-start font-poppins text-[0.6rem] font-black text-blue-500 md:text-xs">
                  0{index + 1}
                </span>
                <span className="relative cursor-pointer font-poppins font-semibold duration-500 before:absolute before:-bottom-2 before:left-0 before:h-0.5 before:w-0 before:bg-blue-500 before:duration-300 hover:before:w-full md:text-xl xl:text-2xl">
                  {item.label}
                </span>
                <ChevronRightIcon className="h-5 w-5 text-blue-500 xl:h-8 xl:w-8" />
              </Link>
            </motion.div>
          ))}
        </motion.div>
